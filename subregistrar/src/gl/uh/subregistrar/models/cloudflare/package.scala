...
package object cloudflare {
  ...
  case class CloudflairErrors(errors: Seq[CloudflairError])
      extends CloudflairResponse[Nothing] {
    def toException =
      new Exception(
        s"Cloudflair errors ${errors.map(e => s"${e.code}: ${e.message}").mkString(", ")}"
      )
  }
  ...
  object CloudflairErrors {
    val reads = Json.reads[CloudflairErrors]
  }
  ...
  object CloudflairResponse {
    implicit def reads[A](implicit aReads: Reads[A]) =
      Reads[CloudflairResponse[A]](js =>
        js \ "success" match {
          case JsDefined(JsBoolean(true)) =>
            js.validate(CloudflairResult.reads[A])
          case JsDefined(JsBoolean(false)) =>
            js.validate(CloudflairErrors.reads)
        }
      )
  }
