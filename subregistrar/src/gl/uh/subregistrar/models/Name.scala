...
object Name {
  ...
  def validate(name: String) = {
    ...
    val violations = (if("^[a-z\\d-]+$".r.matches(lower)) List(s"$lower has invalid characters") else Nil) ++
    (if(name.length > 240) List("name is too long (max length is 240 characters)") else Nil) ++
    Nil
